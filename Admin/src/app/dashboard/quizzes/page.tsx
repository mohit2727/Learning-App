...
export default function QuizzesPage() {
    ...
    // Question-level editing handlers
    const startEditingQuestion = (q: any) => {
        setEditingQuestionId(q._id);
        setEditQuestionText(q.text);
        setEditQuestionOptions([...q.options]);
        setEditQuestionCorrect(q.correctOption);
    };

    const handleSaveQuestion = async (testId: string, questionId: string) => {
        setSavingQuestion(true);
        try {
            const { data } = await api.put(`/tests/${testId}/questions/${questionId}`, {
                text: editQuestionText,
                options: editQuestionOptions,
                correctOption: editQuestionCorrect
            });
            // Update local state
            setViewQuiz(data);
            setQuizzes(quizzes.map(q => q._id === testId ? data : q));
    ...
    const handleDeleteQuestion = async (testId: string, questionId: string) => {
        ...
        try {
            const { data } = await api.delete(`/tests/${testId}/questions/${questionId}`);
            setViewQuiz(data);
            setQuizzes(quizzes.map(q => q._id === testId ? data : q));
        } catch (err) {
    ...
    return (
        <DashboardLayout>
            ...
            {/* View / Edit Quiz Questions Modal */}
            <AnimatePresence>
                {viewQuiz && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                        <motion.div
                            ...
                            animate={{ opacity: 1 }}
                            exit={{ opacity: 0 }}
                            onClick={() => { setViewQuiz(null); setEditingQuestionId(null); }}
                                ...
                                {viewQuiz.questions?.length > 0 ? (
                                    <div className="space-y-4">
                                        {viewQuiz.questions.map((q: any, i: number) => {
                                            const isEditing = editingQuestionId === (q._id || i);
                                            return (
                                                <div key={q._id || i} className={`bg-white border rounded-xl p-4 shadow-sm transition-all ${isEditing ? 'border-blue-300 ring-2 ring-blue-100' : 'border-slate-200'}`}>
                                                    {/* Question header with action buttons */}
                                                    <div className="flex items-start justify-between gap-2 mb-3">
                                                        <div className="flex gap-3 flex-1 min-w-0">
                                                            <span className="text-blue-600 font-black text-sm pt-0.5 shrink-0">{i + 1}.</span>
                                                            {isEditing ? (
                                                                <textarea
                                                                    value={editQuestionText}
                                                                    onChange={(e) => setEditQuestionText(e.target.value)}
                                                                    className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 resize-none transition-all"
                                                                    rows={2}
                                                                />
                                                            ) : (
                                                        ...
                                                        <div className="flex items-center gap-1 shrink-0">
                                                            {isEditing ? (
                                                                <>
                                                                    <button
                                                                        onClick={() => handleSaveQuestion(viewQuiz._id, q._id)}
                                                                    ...
                                                                    <button
                                                                        ...
                                                                        className="p-1.5 bg-slate-50 hover:bg-slate-100 text-slate-400 rounded-lg transition-colors border border-slate-200"
                                                                        title="Cancel"
                                                                    >
                                                                ...
                                                                <>
                                                                    ...
                                                                    <button
                                                                        onClick={() => handleDeleteQuestion(viewQuiz._id, q._id)}
                                                    ...
                                                    <div className="space-y-2 pl-7">
                                                        {(isEditing ? editQuestionOptions : q.options).map((opt: string, optIndex: number) => {
                                                            const isCorrect = isEditing ? editQuestionCorrect === optIndex : q.correctOption === optIndex;
                                                            return (
                                                                <div
                                                                    key={optIndex}
                                                                    className={`p-2.5 rounded-lg border flex items-center gap-3 text-xs font-bold transition-all ${isCorrect
                                                                        ...
                                                                        } ${isEditing ? 'cursor-pointer hover:border-blue-200' : ''}`}
                                                                    onClick={() => { if (isEditing) setEditQuestionCorrect(optIndex); }}
                                                                >
                                                                    <div
                                                                        className={`w-5 h-5 rounded-md flex items-center justify-center text-[10px] font-black shrink-0 ${isCorrect ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-400'}`}
                                                                    >
                                                                    ...
                                                                    {isEditing ? (
                                                                        <input
                                                                            ...
                                                                            className="flex-1 bg-transparent border-none text-sm font-bold text-slate-800 focus:outline-none"
                                                                        />
                                                                    ) : (
                                                                        <span className="flex-1">{opt}</span>
                                                                    )}
                                                                    {isCorrect && <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 ml-auto shrink-0" />}
                                                                </div>
                                                            );
                                                        })}
                                                        {isEditing && (
                                                            <p className="text-[10px] text-blue-500 font-bold mt-1 pl-1">💡 Click an option to mark it as the correct answer</p>
                                                        )}
                                                    </div>
                                                </div>
                                            );
